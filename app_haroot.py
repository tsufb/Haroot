...
if st.button("Generate"):
    ...
    ax.text(0, R_inner / 2, f"קוטר פנימי: {2 * R_inner:.2f} סמ", ha='center', va='center', fontsize=10, color='purple')
    ax.text(x_outer[0] * 0.5, y_outer[0] * 0.5, f"ןרק ךרוא: {L:.2f} מס")
    ax.set_aspect('equal')
    ax.axis('off')
    st.pyplot(fig)
    st.markdown(f"**הקוטר הפנימי (מהמרכז לקשת העליונה):** {2 * R_inner:.2f} סמ")
